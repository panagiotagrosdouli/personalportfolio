  ...
  return (
    <main className="min-h-screen bg-[#fbfaf7] text-stone-950">
      ...
      <section className="mx-auto max-w-5xl px-6 py-16 md:px-10 lg:px-12">
        <p className="section-kicker mb-4">Publications & Research Outputs</p>
        ...
        <p className="mt-6 max-w-3xl text-base leading-8 text-stone-650">
          This page presents publications, manuscripts, technical reports, and research artifacts as they become available. Until peer-reviewed publications are listed, the emphasis is on transparent research progress: well-scoped questions, reproducible project documentation, technical reports, and open artifacts.
        </p>
        ...
        <div className="mt-12 divide-y divide-stone-200 border-y border-stone-200">
          {outputs.map((item) => (
            <div key={item.title} className="grid gap-4 py-7 md:grid-cols-[220px_1fr]">
              <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-stone-400">{item.title}</p>
              <p className="text-sm leading-7 text-stone-600">{item.text}</p>
            </div>
          ))}
        </div>

        <div className="mt-12 border-l border-emerald-900/40 pl-6">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-900">Current priority</p>
          <p className="mt-3 max-w-2xl text-lg leading-9 text-stone-700">

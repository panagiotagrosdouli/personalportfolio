    ...
    <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      ...
      <article className="mx-auto max-w-7xl px-5 py-12 md:px-8 md:py-20">
        ...
        {project.evidence?.length ? (
          <Section label="Measured evidence">
            <div className="grid gap-5 lg:grid-cols-3">
              {project.evidence.map((artifact) => (
                <figure key={artifact.src} className="overflow-hidden rounded-[1.5rem] border border-[var(--line)] bg-[var(--panel)]">
                  ...
                  <figcaption className="border-t border-[var(--line)] p-5">
                    <h2 className="font-serif text-2xl tracking-[-0.04em]">{artifact.title}</h2>
                    <p className="mt-3 text-sm leading-7 text-[var(--muted)]">{artifact.caption}</p>
                  </figcaption>
                </figure>
              ))}
            </div>
            <p className="mt-5 max-w-4xl text-xs leading-6 text-[var(--muted)]">
              These figures are generated by the diploma-thesis codebase. They document its current controlled experimental scope and do not imply deployment in a physical intersection.
            </p>
          </Section>
        ) : null}
